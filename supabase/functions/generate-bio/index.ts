...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { subject, photos } = await req.json()
    ...
    // 最多取 6 张有图的记录用于视觉理解
    const imagePhotos = (photos as any[]).filter((p: any) => p.image_url).slice(0, 6)

    // 文字摘要：所有记录
    const textContext = (photos as any[]).map((p: any) => {
...
只返回 JSON，不要任何解释：{"tagline":"...","bio":"..."}`
    ...
    for (const photo of imagePhotos) {
      content.push({
        type: 'image_url',
        image_url: { url: photo.image_url, detail: 'low' },
      })
    }
